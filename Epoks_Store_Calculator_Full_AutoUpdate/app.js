...
async function fetchRemoteValuesIfConfigured(){
  const url = (settings.valuesUrl||'').trim();
  if(!url) return;
  try{
    const res = await fetch(url, {cache:'no-store'});
    if(!res.ok) throw new Error('HTTP ' + res.status);
    const json = await res.json();
    if(!json.items || !Array.isArray(json.items)) throw new Error('JSON missing items[]');
    // Accept either {items:[{item,cashValue,...}]} or {values:[...]}
    const items = json.items || json.values;
    // Normalize
    DATA.values = items.map(x => ({
      item: x.item || x.name || x.id || x.Item || x.title,
      cashValue: Number(x.cashValue ?? x.value ?? x.cash ?? x.CashValue)
    })).filter(x => x.item && Number.isFinite(x.cashValue));
    DATA.valuesSource = url;
    DATA.generatedAt = new Date().toISOString();
    toast('Values updated');
  }catch(e){
    toast('Could not fetch values: ' + e.message);
  }
}
